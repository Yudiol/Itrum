...
public class ProductController {
    ...
    public String getAll() {
        ...
        try {
            return objectMapper.writeValueAsString(listProducts);
    ...
    public Long create(HttpServletRequest request) throws IOException {
        ...
        Product product = objectMapper.readValue(stringBuilder.toString(), Product.class);
        return productService.create(product);
    }

    @PatchMapping("/{productId}")
    public Long update(HttpServletRequest request) throws IOException {
        String requestURI = request.getRequestURI();
        String[] pathSegments = requestURI.split("/");
        Long productId = Long.parseLong(pathSegments[pathSegments.length - 1]);
        StringBuilder stringBuilder = new StringBuilder();
